...
export function useResortLists() {
  ...
  const filtered = useMemo(() => filterResorts(resorts, share, context), [share, context]);
  ...
  const relaxed = useMemo(() => {
    ...
    return sortResorts(filterResorts(resorts, loose, context), "distance", "asc", distanceOf).slice(0, 3);
  }, [filtered.length, share, context, distanceOf]);
